...
|---|---|---|---|
| R1 | handled | Planned artifact `research/adapter-history.md` resolved the authored-versus-merge ambiguity: `54aef287` is the first broken authored tree, while `80fad2bd` is PR #352's merge that first introduced it to `dev`; independent merge-parent, ancestry, relevant-tree, and build checks corroborate the result. | independent verifier |
| R2 | handled | Planned artifact `research/engine-package-lineage.md` resolved publication provenance: restored preview.19 names source commit `88be469`, its fresh/cache hashes match, and its DLL lacks `HandshakeFor`, so version text was not treated as publication proof. | independent verifier |
