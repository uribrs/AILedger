...
## Repairs round 3 (post full-scan) + final parity
- A: duplicate-ID crash fixed (seen-set dedup → one record); detection parser CheckCharacters=false; cursor-loop termination guards (max-page + non-advancing-URL) on all 3 fetch loops. 10/10 tests.
- B: detection AND spine fetch both follow <WARNING>/<URL> cursor + per-host merge (full spine parity with A); _collectedAssets counts emitted + shortfall warning; termination guards on both loops. 5/5 fast tests.
- A: asset count now emitted-based, accumulated across batches + checkpoint/resume + 1960-degrade path (parity with B); spine-shortfall warning. 10/10.
- B: duplicate-ID dedup (seen-set, one record, shared-ref hazard removed) + dup-ID test. 5/5.

## Final parity (verifier-2 + verifier-3 + targeted fixes): CLOSED
Detection cursor+merge, spine cursor+merge, termination guards, duplicate-ID→one-record, emitted-asset-count, record shape — all equivalent A↔B.
Accepted residual (cosmetic, framework): B identity-less host logs at Info (eLogLevels has no Warning tier); A logs Warning via ILogger. Not a defect.

## Live validation closes the ids= caveat
The user's local run (logs/published-batches/20260610-071549) exercised A's per-batch GetHostDetailsAsync(ids=...&details=All) against the real tenant and produced correct full HOST_DETAILS for all 7 hosts — empirically confirming the host-list ids= filter (previously doc-only).

## Accepted / out-of-scope (user-confirmed or framework)
- Parser drops vulnerability findings with empty cve_ids (pre-existing; USER SAID LEAVE AS IS).
- B stress test suite not runnable here (VSTest harness hang); fast/targeted tests green.
- Parser repo on branch defender-vm-handle-numbered-findings-files (HOUSEKEEPING: move before commit).

## FINAL STATUS: complete. 3 repos changed, both review rounds + full-scan + parity all closed. Nothing committed/pushed.
