...
internal static class GovernedExecutionBriefing
{
    public static string For(AgentLaunchRequest request, string ledgerRoot)
    {
        ...
        builder
            ...
            .AppendLine()
            .AppendLine("Researchers: save research/<topic>.md as supporting output and record findings through")
            .AppendLine("claim add and evidence add with underlying source citations and --supports or --refutes.")
            .AppendLine("There is no Research artifact kind. Do not call artifact record for a research report;")
            .AppendLine("researcher completion does not require an artifact. Report evidence IDs and unresolved")
            .AppendLine("questions to the caller; claim resolution belongs to an authorized operator or lead.")
            .AppendLine()
            .AppendLine("For roles with required artifact outputs, only the producing run can file them. The kernel")
            ...
            .AppendLine("verifier or review output always does. File the artifacts required for your role and stage")
            .AppendLine("before stopping; missing required artifacts block the corresponding completion or stage gate.")
            .AppendLine()
            ...
            .AppendLine("    structured provider output. AILedger preserves complete JSON records, with an 8 MiB limit")
            .AppendLine("    per stream and across retained output. Treat these as integration limits, not vendor guarantees.")
            ...
            .AppendLine("    JSON line can still exceed the total budget.")
            .AppendLine()
