...
| 6 | see-inside-a-run | `2026-09-08_1428-run-cost` | W1 done, W2 next | **blocks 9.** `AgentRunResult` carries the provider's own event stream — turns, tool calls, tokens — and `CliApplication.cs:939` writes it to stdout and drops it. 114 runs, 0 models recorded. the governance-cost dimension is unmeasurable without it. |
